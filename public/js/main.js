...
function scrollTrack(dir) {
    ...
    const cardWidth = 345;
    const maxOffset = -(track.children.length - 3) * cardWidth;
...
function renderProducts(category = 'all') {
    ...
    const filtered = category === 'all' ? productsData : productsData.filter(p => p.cat === category);
    
    grid.innerHTML = filtered.map(p => {
        const badgeMap = { sale: '<span class="badge badge-sale">Sale</span>', new: '<span class="badge badge-new">New</span>', hot: '<span class="badge badge-hot">Hot</span>', bestseller: '<span class="badge badge-bestseller">Bestseller</span>' };
...
function subscribeNewsletter() {
    ...
    showToast('🎉 Subscribed! Check your email for 10% discount code.');
